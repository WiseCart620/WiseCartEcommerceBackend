...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    private boolean isPublicEndpoint(HttpServletRequest request) {
        ...
        return checkPublicPatterns(uri, method) || checkPublicPatterns(normalizedUri, method);
    }

    private boolean checkPublicPatterns(String uri, String method) {
        // Auth endpoints
        ...
        if (uri.startsWith("/categories/public") || uri.startsWith("/api/categories/public")
                || uri.equals("/categories") || uri.equals("/api/categories")
                || // GET all categories is public
                uri.startsWith("/categories/tree") || uri.startsWith("/api/categories/tree")
                || uri.matches("/categories/\\d+") || uri.matches("/api/categories/\\d+")
                || // GET by ID
        ...
        if ("GET".equalsIgnoreCase(method)
                && (uri.startsWith("/reviews") || uri.startsWith("/api/reviews"))) {
            return true;
        }
