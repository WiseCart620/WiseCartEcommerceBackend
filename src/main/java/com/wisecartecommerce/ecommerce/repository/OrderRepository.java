...
public interface OrderRepository extends JpaRepository<Order, Long> {
    ...
    @Query(value = "SELECT o FROM Order o LEFT JOIN FETCH o.user u "
            ...
            + "(:customerEmail IS NULL OR (u IS NOT NULL AND u.email = :customerEmail) OR o.guestEmail = :customerEmail)",
            countQuery = "SELECT COUNT(DISTINCT o) FROM Order o LEFT JOIN o.user u "
            + "WHERE (:status IS NULL OR o.status = :status) AND "
            + "(:startDate IS NULL OR o.createdAt >= :startDate) AND "
            + "(:endDate IS NULL OR o.createdAt <= :endDate) AND "
            + "(:customerEmail IS NULL OR (u IS NOT NULL AND u.email = :customerEmail) OR o.guestEmail = :customerEmail)")
    ...
    @Query("SELECT COUNT(o) FROM Order o WHERE o.status IN :statuses AND "
            + "o.trackingNumber IS NOT NULL AND o.trackingNumber LIKE 'P%'")
    Long countReadyForPickup(@Param("statuses") List<OrderStatus> statuses);
